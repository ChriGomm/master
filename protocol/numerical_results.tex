...
\end{align*}
Now only the terms coupling the two direction of phonon modes on the same side have to be tuned to zero. Because of the nearest neighbour nature of the consideration only to distances play a role, the intracell distance $\mathbf{r}_I$ and the intercell distance $\mathbf{r}_O$. 
\begin{align*}
...
\end{align*}
The dipoles are then chosen in such a way that this sum vanishes.
...
\end{align*}
where $a$ is the lattice constant, and the parameters $b$ and $h$ can be chosen to realize different lattice configurations. $b$ and $h$ have to be chosen in a way, such that the nearest neighbour restriction makes sense. 
