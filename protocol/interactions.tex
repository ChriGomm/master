...
\begin{figure}[H]
    \hspace*{-2cm}
    \includegraphics{pictures/tuning_first_order_to_zero2.pdf}
...
\end{align*}
This allows for a topological characterization of the bosonic system, acording to \cite{peanoTopologicalEdgeStates2018}. The Heisenberg equations of motion for the operators read
\begin{align*}
...
Fourier transforming a state $\ket{\psi}=\ket{\psi}_\text{int}\psi(y)\in\mathfrak{h}$, $\ket{\phi}=\ket{\phi}_\text{int}\phi(y)\in\overline{\mathfrak{h}}$, where $y$ is the position of a unit cell, is defined as 
\begin{align*}
    \mathcal{F}(\psi)(k)&=\sum_y e^{-iky}\ket{\psi}_\text{int}\psi(y)\\
    \mathcal{F}(\phi)(k)&=\sum_y e^{iky}\ket{\phi}_\text{int}\phi(y)
\end{align*}
By this one can determine the Fourier transform of the operators by their action on the states.
\begin{align*}
    \mathcal{F}(\mathbf{h}\psi)(k)&=\sum_{y,x}e^{-iky}\,h_x u^x \ket{\psi}_\text{int}\psi(y)\\
    &=\sum_{y,x}e^{-iky}\,h_x \ket{\psi}_\text{int}\psi(y-x)\\
    &=\sum_{z,x}e^{-ikz}e^{-ikx}\,h_x \ket{\psi}_\text{int}\psi(z)\\
...
\begin{align*}
    \mathcal{F}(\overline{\mathbf{h}}\phi)(k)&=\sum_{y,x}e^{iky}\,\bar{h}_x u^x \ket{\phi}_\text{int}\phi(y)\\
...
\begin{align*}
     \mathcal{F}(\mathbf{\Delta}_s\phi)(k)&=\sum_{y,x}e^{-iky}\,\Delta_x u^x \ket{\phi}_\text{int}\phi(y)\\
    &=\sum_{x}e^{-ikx}\,\Delta_x \mathcal{F}(\phi)(-k)\\
    &= \mathbf{\Delta}_s(k)\,\phi(-k)\\
    \mathcal{F}(\mathbf{\Delta}_s^\dagger\psi)(k)&=\sum_{y,x}e^{iky}\,\Delta_x^\dagger u^x \ket{\psi}_\text{int}\psi(y)\\
...
\begin{figure}[H]
    \centering
    \includegraphics{pictures/Bdg_spec_80N.pdf}
    \caption{\textbf{BdG spectrum for lattice size 80}}
\end{figure}
\begin{figure}[H]
    \centering
    \includegraphics{pictures/Bdg_spec_800N.pdf}
