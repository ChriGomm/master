...
One can, however, also consider a regime where the motional excitations are not conserved, i.e. $\omega\lesssim|H|$. For this investigation I assume dipole-dipole interactions between mixed Rydberg states. The corresponding potential reads
\begin{align*}
...
\end{align*}
Here $h$ and $\tilde{U}$ are dimensionless and the the term $m\omega^2 a^2/[V_0(a)\,\text{max}(\tilde{U})]$ chosen of order $0.01$-$250$. Now I use the two free parameters $h$ and the angle $\varphi_d$ of the dipole moment $\hat{\mathbf{d}}$ to set $G_i=0$ or close to zero for different values of $b$. Close to zero means here that max$(|G_i|)$ compared to $\sqrt{\hbar/(2m\omega)}\,\text{max}(\tilde{U})/a$ is small. Here $\sqrt{\hbar/(2m\omega)}/a\sim10^{-3}$ is the displacement from a single motional excitation in units of $a$.
\begin{figure}[H]
    \hspace*{-2cm}
...
\end{align*}
with $\vec{a}=(a_1,\dots,a_N,a_1^\dagger,\dots,a_N^\dagger)$ the vector of operators and a coupling matrix $A$, which takes the following form
\begin{align*}
...
\begin{align*}
    ...
    \text{where}\quad \tilde{H} &= \frac{\hbar}{m\omega}\,\begin{pmatrix}
       ...
       \end{pmatrix}
\end{align*}
And it is this matrix $\tilde{H}$ that determines the topological properties of the system. $\tilde{H}$ is in the Bogoliubov de Gennes form and a real matrix. Therefore it is time reversal symmetric. Particle-hole symmetry is also fulfilled, as $\tilde{H}^*=-\Sigma_x\,\tilde{H}\,\Sigma_x$ with 
