...
\end{align*}
where $\mathbf{P}_i$ is the momentum of the $i$th atom. I can now redefine the trap centers by a equal shift of all positions $\{\delta\mathbf{R}_l\}=\{\delta\tilde{\mathbf{R}}_l+\mathbf{\Delta}\}$, where $\delta\mathbf{R}_l$ and $\delta\tilde{\mathbf{R}}_l$ are operators and $\mathbf{\Delta}$ is just a constant vector. Neglecting all constant terms the Hamiltonian minus the momentum term $\tilde{H}=H-\sum_i\mathbf{P}_i/2m$ up to second order reads
\begin{align*}
