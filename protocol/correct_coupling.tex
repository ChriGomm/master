...
\subsection*{Spin-Phonon Hamiltonian}
...
% \begin{align*}
%     H_\text{int}&= \sum_{i=1}^{N-1}\left[V(\mathbf{r}_{i,i+1}) + \sum_{n\in\{i,i+1\}}\frac{\partial V(\mathbf{r}_{i,i+1})}{\partial\mathbf{R}_{n}}\,\delta\mathbf{\hat{R}}_n +\frac12\,\sum_{n,m\in\{i,i+1\}}\frac{\partial^2V(\mathbf{r}_{i,i+1})}{\partial\mathbf{R}_{n}\partial\mathbf{R}_m}\,\delta\mathbf{\hat{R}}_n\delta\mathbf{\hat{R}}_m\right]\,\sigma_i^+\sigma_{i+1}^-+\text{h.c.}\\
%     &= \sum_{i=1}^{N-1}\Bigg[V(\mathbf{r}_{i,i+1}) + \frac{\partial V(\mathbf{r}_{i,i+1})}{\partial\mathbf{r}_{i,i+1}}\,(\delta\mathbf{\hat{R}}_{i+1}-\delta\mathbf{\hat{R}}_i) \\
%     &\quad\quad\quad+\frac12\,\frac{\partial^2V(\mathbf{r}_{i,i+1})}{\partial\mathbf{r}_{i,i+1}^2}\,\left(\delta\mathbf{\hat{R}}_i^2+\delta\mathbf{\hat{R}}_{i+1}-\delta\mathbf{\hat{R}}_i\,\delta\mathbf{\hat{R}}_{i+1}-\delta\mathbf{\hat{R}}_{i+1}\delta\mathbf{\hat{R}}_i\right)\Bigg]\,\sigma_i^+\sigma_{i+1}^-+\text{h.c.}
% \end{align*}
% As this step one can introduce a shift in the displacement as was done in \secref{sec:Physical Implementation},i.e. $\{\delta\mathbf{R}_l\}\rightarrow\{\delta{\mathbf{R}}_l+\mathbf{\Delta}_l\}$. Plugging the shifted displacements in the above equation and considering only the terms linear in displacement ($H_\text{Lin}$), one can collect the following terms.
% \begin{align*}
%     H_\text{Lin}&= \sum_{i=1}^{N-1}\Bigg[\left(- \frac{\partial V(\mathbf{r}_{i,i+1})}{\partial\mathbf{r}_{i,i+1}}-\frac{\partial^2V(\mathbf{r}_{i,i+1})}{\partial\mathbf{r}_{i,i+1}^2}\,({\Delta}_{i+1}-{\Delta}_i)\right)\,\delta\mathbf{\hat{R}}_{i} \\
%     &\quad\quad\quad+\left( \frac{\partial V(\mathbf{r}_{i,i+1})}{\partial\mathbf{r}_{i,i+1}}+\frac{\partial^2V(\mathbf{r}_{i,i+1})}{\partial\mathbf{r}_{i,i+1}^2}\,({\Delta}_{i+1}-{\Delta}_i)\right)\,\delta\mathbf{\hat{R}}_{i+1}\Bigg]\,\sigma_i^+\sigma_{i+1}^-+\text{h.c.}\\
%     &= \sum_{i=1}^{N-1}\Bigg[\frac{V_0}{a}\,\left(\frac{a^4}{r_{i,i+1}^4}\,g(\mathbf{\hat{r}}_{i,i+1})+\frac{a^5}{r_{i,i+1}^5}\,c(\mathbf{\hat{r}}_{i,i+1})\,\left(\frac{{\Delta}_{i+1}}{a}-\frac{{\Delta}_i}{a}\right)\right)\,(\delta\mathbf{\hat{R}}_{i+1}-\delta\mathbf{\hat{R}}_{i}) \Bigg]\,\sigma_i^+\sigma_{i+1}^-+\text{h.c.}
% \end{align*}
% This constitutes the condition which the shifts in displacement have to satisfy.
% \begin{align*}
%      G_i\vcentcolon=\frac{V_0}{a}\,\left(\frac{a^4}{r_{i,i+1}^4}\,g(\mathbf{\hat{r}}_{i,i+1})+\frac{a^5}{r_{i,i+1}^5}\,c(\mathbf{\hat{r}}_{i,i+1})\,\left(\frac{{\Delta}_{i+1}}{a}-\frac{{\Delta}_i}{a}\right)\right)=0\quad \forall i\in\{1,N-1\}
% \end{align*}
% With this definition one can write the interaction Hamiltonian as
% \begin{align*}
%     H_\text{int}&= \sum_{i=1}^{N-1}\Bigg[V(\mathbf{r}_{i,i+1}) + \Delta V_i + G_i\,(\delta\mathbf{\hat{R}}_{i+1}-\delta\mathbf{\hat{R}}_i) \\
%     &\quad\quad\quad+\frac12\,\frac{\partial^2V(\mathbf{r}_{i,i+1})}{\partial\mathbf{r}_{i,i+1}^2}\,\left(\delta\mathbf{\hat{R}}_i^2+\delta\mathbf{\hat{R}}_{i+1}-\delta\mathbf{\hat{R}}_i\,\delta\mathbf{\hat{R}}_{i+1}-\delta\mathbf{\hat{R}}_{i+1}\delta\mathbf{\hat{R}}_i\right)\Bigg]\,\sigma_i^+\sigma_{i+1}^-+\text{h.c.}\\
%     \text{with}\quad \Delta V_i &= V_0\,\frac{a^4}{r_{i,i+1}^4}\,g(\mathbf{\hat{r}}_{i,i+1})\,\left(\frac{{\Delta}_{i+1}}{a}-\frac{{\Delta}_i}{a}\right)+\frac12\,V_0\,\frac{a^5}{r_{i,i+1}^5}\,c(\mathbf{\hat{r}}_{i,i+1})\,\left(\frac{{\Delta}_{i+1}}{a}-\frac{{\Delta}_i}{a}\right)^2
% \end{align*}
% \begin{figure}[H]
%     \hspace*{-2cm}
%     \includegraphics{pictures/tuning_first_order_to_zero_spph.pdf}
%     \caption{\textbf{Parameters for negligible first order.} }
%     \label{fig:tuning_first_order_to_zero_spph}
% \end{figure}

\begin{align*}
    H_\text{int}&= \sum_{i,j=1;\,i<j}^{N-1}\left[V(\mathbf{r}_{i,j}) + \sum_{n\in\{i,j\}}\frac{\partial V(\mathbf{r}_{i,j})}{\partial\mathbf{R}_{n}}\,\delta\mathbf{\hat{R}}_n +\frac12\,\sum_{n,m\in\{i,j\}}\frac{\partial^2V(\mathbf{r}_{i,j})}{\partial\mathbf{R}_{n}\partial\mathbf{R}_m}\,\delta\mathbf{\hat{R}}_n\delta\mathbf{\hat{R}}_m\right]\,\sigma_i^+\sigma_{j}^-+\text{h.c.}\\
    &= \sum_{i,j=1; \,i<j}^{N-1}\Bigg[V(\mathbf{r}_{i,j}) + \frac{\partial V(\mathbf{r}_{i,j})}{\partial\mathbf{r}_{i,j}}\,(\delta\mathbf{\hat{R}}_{j}-\delta\mathbf{\hat{R}}_i) \\
    &\quad\quad\quad+\frac12\,\frac{\partial^2V(\mathbf{r}_{i,j})}{\partial\mathbf{r}_{i,j}^2}\,\left(\delta\mathbf{\hat{R}}_i^2+\delta\mathbf{\hat{R}}_{j}-\delta\mathbf{\hat{R}}_i\,\delta\mathbf{\hat{R}}_{j}-\delta\mathbf{\hat{R}}_{j}\delta\mathbf{\hat{R}}_i\right)\Bigg]\,\sigma_i^+\sigma_{j}^-+\text{h.c.}
\end{align*}
As this step one can introduce a shift in the displacement as was done in \secref{sec:Physical Implementation},i.e. $\{\delta\mathbf{R}_l\}\rightarrow\{\delta{\mathbf{R}}_l+\mathbf{\Delta}_l\}$. Plugging the shifted displacements in the above equation and considering only the terms linear in displacement ($H_\text{Lin}$), one can collect the following terms.
\begin{align*}
    H_\text{Lin}&= \sum_{i,j=1;\,i<j}^{N-1}\Bigg[\left(- \frac{\partial V(\mathbf{r}_{i,j})}{\partial\mathbf{r}_{i,j}}-\frac{\partial^2V(\mathbf{r}_{i,j})}{\partial\mathbf{r}_{i,j}^2}\,({\Delta}_{j}-{\Delta}_i)\right)\,\delta\mathbf{\hat{R}}_{i} \\
    &\quad\quad\quad+\left( \frac{\partial V(\mathbf{r}_{i,j})}{\partial\mathbf{r}_{i,j}}+\frac{\partial^2V(\mathbf{r}_{i,j})}{\partial\mathbf{r}_{i,j}^2}\,({\Delta}_{j}-{\Delta}_i)\right)\,\delta\mathbf{\hat{R}}_{j}\Bigg]\,\sigma_i^+\sigma_{j}^-+\text{h.c.}\\
    &= \sum_{i,j=1;\,i<j}^{N-1}\Bigg[\frac{V_0}{a}\,\left(\frac{a^4}{r_{i,j}^4}\,g(\mathbf{\hat{r}}_{i,j})+\frac{a^5}{r_{i,j}^5}\,c(\mathbf{\hat{r}}_{i,j})\,\left(\frac{{\Delta}_{j}}{a}-\frac{{\Delta}_i}{a}\right)\right)\,(\delta\mathbf{\hat{R}}_{j}-\delta\mathbf{\hat{R}}_{i}) \Bigg]\,\sigma_i^+\sigma_{j}^-+\text{h.c.}
\end{align*}
This constitutes the condition which the shifts in displacement have to satisfy.
\begin{align*}
     G_{i,j}\vcentcolon=\frac{V_0}{a}\,\left(\frac{a^4}{r_{i,j}^4}\,g(\mathbf{\hat{r}}_{i,j})+\frac{a^5}{r_{i,j}^5}\,c(\mathbf{\hat{r}}_{i,j})\,\left(\frac{{\Delta}_{j}}{a}-\frac{{\Delta}_i}{a}\right)\right)=0\quad \forall i,j\in\{1,N-1\};\;i<j
\end{align*}
With this definition one can write the interaction Hamiltonian as
\begin{align*}
    H_\text{int}&= \sum_{i,j=1;\,i<j}^{N-1}\Bigg[V(\mathbf{r}_{i,j}) + \Delta V_{i,j} + G_{i,j}\,(\delta\mathbf{\hat{R}}_{j}-\delta\mathbf{\hat{R}}_i) \\
    &\quad\quad\quad+\frac12\,\frac{\partial^2V(\mathbf{r}_{i,j})}{\partial\mathbf{r}_{i,j}^2}\,\left(\delta\mathbf{\hat{R}}_i^2+\delta\mathbf{\hat{R}}_{j}-\delta\mathbf{\hat{R}}_i\,\delta\mathbf{\hat{R}}_{j}-\delta\mathbf{\hat{R}}_{j}\delta\mathbf{\hat{R}}_i\right)\Bigg]\,\sigma_i^+\sigma_{j}^-+\text{h.c.}\\
    \text{with}\quad \Delta V_{i,j} &= V_0\,\frac{a^4}{r_{i,j}^4}\,g(\mathbf{\hat{r}}_{i,j})\,\left(\frac{{\Delta}_{j}}{a}-\frac{{\Delta}_i}{a}\right)+\frac12\,V_0\,\frac{a^5}{r_{i,j}^5}\,c(\mathbf{\hat{r}}_{i,j})\,\left(\frac{{\Delta}_{j}}{a}-\frac{{\Delta}_i}{a}\right)^2
\end{align*}
\begin{figure}[H]
    \hspace*{-2cm}
    \includegraphics{pictures/tuning_first_order_to_zero_spph.pdf}
