...
\begin{align*}
    B&= \sum_{i_1=1}^{\text{dim}(\mathfrak{h})}\sum_{i_2=1}^{i_1}\cdots\sum_{i_L}^{i_{L-1}}\sum_{j_1=1}^{\text{dim}(\mathfrak{h})}\sum_{j_2=1}^{j_1}\cdots\sum_{j_K=1}^{j_{K-1}}\frac{\sqrt{L!\,K!}}{\sqrt{n_{1}!\cdots n_{L}!\,m_{1}!\cdots m_{K}!}}\\\\&\cdot\braket{P^L_\pm\left(l_{i_1}\otimes\cdots\otimes l_{i_L}\right)|B_{K,L}|P^K_\pm\left(k_{j_1}\otimes\cdots\otimes k_{j_K}\right)} a_{j_1}^\dagger\cdots a_{i_L}^\dagger a_{j_1}\cdots a_{j_K}
\end{align*}
For an operator $B_{2,0}$ with $K=2$, $L=0$ this takes the explicit form
\begin{align}\label{eq:fock_2body}
    B&= \sum_{j_1=1}^{\text{dim}(\mathfrak{h})}\sum_{j_2=1}^{j_1} \frac{\sqrt{2}}{\sqrt{n_1!\,n_2!}} B_{2,0}\ket{P_\pm^2(k_{j_1}\otimes k_{i_2})}\,a_{i_1}\,a_{i_2}
\end{align}
In order to validate this for the above example, take the lifted operator $D_2$ with
\begin{align*}
    D_2&= \sum_{\alpha,\beta}\bra{\phi_\alpha}\bra{\phi_\beta}\\
    D^{(M)}&= \frac{1}{\sqrt{M(M-1)}}\,\sum_{\alpha,\beta}\,\sum_{i,j=1;\,i\neq j}^M\bigotimes_{k=1}^M \left[\mathbb{1}+\delta_{k,i}\,(\bra{\phi_\alpha}-\mathbb{1})+\delta_{k,j}\,(\bra{\phi_\beta}-\mathbb{1})\right]
\end{align*}
...
\begin{align*}
    D^{(M)}\ket{\Phi}&=\half\,\sqrt{M\,(M-1)}\,\sum_{\alpha,\beta}\bra{\phi_\alpha}\otimes\bra{\phi_\beta}\otimes\mathbb{1}\otimes\cdots\otimes\mathbb{1}\ket{\Phi}\\
    &=\half\,\sum_{\alpha,\beta}\sqrt{M-1}\,\braket{\phi_\alpha|\Phi'}_\text{partial}\quad \quad\quad\text{with }\ket{\Phi'}=\sqrt{M}\,\braket{\phi_\beta|\Phi}_\text{partial}\\
    &=\half\,\sum_{\alpha,\beta} a_\alpha\,a_\beta\,\ket{\Phi}
\end{align*}
...
\begin{alignat*}{2}
    A = (\mathbf{a}^\dagger)^t\mathbf{A}\,\mathbf{a}\quad\quad&&B = (\mathbf{a}^\dagger)^t\mathbf{B}\,\mathbf{a}
\end{alignat*}
For the bosonic case these matrices take the form
\begin{alignat*}{2}
    \mathbf{A}=\left(\braket{e_i|A_s|e_j}\right)_{i,j}\quad\quad\quad&&\mathbf{B}=\left(B_{2,0}\ket{P_\pm^2(k_{i}\otimes k_{j})}\,(1+\delta_{i,j})/2\right)_{i,j}
\end{alignat*}
...
\end{align*}  
In summary these are the two most significant properties of topological protected non-trivial phases, i.e. there are eigenenergies of the open system, whose states are exponentially localized at the boundary.
...
\begin{align*}
    h'&= \Big( \braket{b_i|\mathbf{h}|b_j} \Big)_{i,j}\\
    \overline{h}'&=\Big( \braket{c_i|\overline{\mathbf{h}}|c_j} \Big)_{i,j}\\
    \Delta'&=\Big( \braket{b_i|\mathbf{\Delta}|c_j} \Big)_{i,j}\\
    (\Delta^\dagger)'&=\Big( \braket{c_i|\mathbf{\Delta}^\dagger|b_j} \Big)_{i,j}
\end{align*}
...
\begin{align*}
    ...
    &= {\Delta}_s^\dagger(k)\,\psi(-k)
\end{align*}
If we order the space in such a form that $\ket{\psi}=\int_{\mathbb{}}\oplus_k (\psi(k),\phi(-k))$ one can write the Hamiltonian as block diagonal $\mathcal{H}=\oplus_k \mathbf{A_k}$ with
\begin{align*}
    \mathbf{A_k} = \begin{pmatrix}
        {h}(k) & {\Delta}_s(k)\\
        {\Delta}_s^\dagger(-k) & \overline{{h}}(-k)
    \end{pmatrix}
\end{align*}
