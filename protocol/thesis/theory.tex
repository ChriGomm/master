...
\begin{align}\label{eq:fock_2body}
    B&= \sum_{j_1=1}^{\text{dim}(\mathfrak{h})}\sum_{j_2=1}^{j_1} B_{2,0}\ket{P_\pm^2(k_{j_1}\otimes k_{i_2})}\,a_{i_1}\,a_{i_2}
\end{align}
Both \eqref{eq:fock_single_body} and \eqref{eq:fock_2body} can be written in matrix after introducing the vector $\mathbf{a}=(a_1,\dots,a_{\text{dim}(\mathfrak{h})})$ and defining appropriate matrices $\mathbf{A}$ and $\mathbf{B}$.
\begin{alignat*}{2}
